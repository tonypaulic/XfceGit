...
build () {
	...
	echo
	echo "================================================================" | tee -a $LOG
	echo "Building: $1" | tee -a $LOG
	echo "================================================================" | tee -a $LOG
	cd $SOURCE_DIR/$1	
		echo ">>>>>SETUP:" >> $LOG
	meson setup --reconfigure build --prefix=$PREFIX $2 | tee -a $LOG
		echo ">>>>>COMPILE:" >> $LOG
	meson compile -C build | tee -a $LOG
		echo ">>>>>INSTALL:" >> $LOG
	sudo meson install -C build | tee -a $LOG
}
...
case $1 in
	...
	info)
		...
		do
			v=$(version $x)	
			echo "  $x = $v" | sed -e 's/.git//' | tee -a $LOG
		done
	...
	init)
		touch $LOG
			echo "xfceGIT.sh: Running with init paramter." >> $LOG
			echo "=======================================" >> $LOG
			echo "" >> $LOG
		...
		do
			git clone https://gitlab.xfce.org/xfce/$package | tee -a $LOG
		done

		for package in $XFCE_PLUGINS
		do
			git clone https://gitlab.xfce.org/panel-plugins/$package | tee -a $LOG
		done

		for package in $THUNAR_PLUGINS
		do
			git clone https://gitlab.xfce.org/thunar-plugins/$package | tee -a $LOG
		done

		for package in $XFCE_APPS
		do
			git clone https://gitlab.xfce.org/apps/$package | tee -a $LOG
		done
		...
		sudo pacman -S \
			...
			polkit xorg-iceauth \
			libxpresent \
			libyaml gst-plugin-gtk gst-plugins-{bad,ugly} \
	...
	update|update-all)
		touch $LOG
			echo "xfceGIT.sh: Running with update parameter." >> $LOG
			echo "==========================================" >> $LOG
			echo "" >> $LOG
		...
		(
			...
			cd $SOURCE_DIR/xfce4-dev-tools
				echo -n "Building xfce4-dev-tools..." >> $LOG
			make clean
			./autogen.sh    --prefix=/usr \
				             --sysconfdir=/etc \
				             --libexecdir=/usr/lib | tee -a $LOG
			make | tee -a $LOG
		...
		(
			...
			### 
			cd $SOURCE_DIR/exo
				echo -n "Building exo..." >> $LOG
			make clean
			./autogen.sh    --prefix=/usr | tee -a $LOG
			make | tee -a $LOG
			sudo make install | tee -a $LOG
				echo "done." >> $LOG
		)
		...
		### xfdesktop = libyaml gst-plugin-gtk gst-plugins-{bad,ugly}
		...
		### 
		echo $xXFCE_PLUGINS | grep xfce4-fsguard-plugin && build xfce4-fsguard-plugin 
		###                
...
cd
.bashrc = 	export WLR_RENDERER_ALLOW_SOFTWARE=1
				export WLR_NO_HARDWARE_CURSORS=1
startxfce4 --wayland wayfire
################################### Wayand/labwc instructions
pacman -Sy labwc
startxfce4 --wayland 
################################### 
